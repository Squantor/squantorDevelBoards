...
void boardInit(void)
{
    ...
    SwmMovablePinAssign(SWM, SWM_USART0_TXD_O, PIN_UART_TX);
    SwmMovablePinAssign(SWM, SWM_USART0_RXD_I, PIN_UART_RX);

    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_SIO0, PIN_MODE_INACTIVE);
    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_SIO1, PIN_MODE_INACTIVE);
    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_SIO2, PIN_MODE_INACTIVE);
    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_SIO3, PIN_MODE_INACTIVE);
    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_CE, PIN_MODE_INACTIVE);
    IoconPinSetMode(IOCON, IOCON_SPI_FLASH_SCK, PIN_MODE_INACTIVE);
    SwmMovablePinAssign(SWM, SWM_SPI0_MOSI_IO, PIN_SPI_FLASH_SIO0);
    SwmMovablePinAssign(SWM, SWM_SPI0_MISO_IO, PIN_SPI_FLASH_SIO1);
    SwmMovablePinAssign(SWM, SWM_SPI0_SCK_IO, PIN_SPI_FLASH_SCK);
    SwmMovablePinAssign(SWM, SWM_SPI0_SSEL0_IO, PIN_SPI_FLASH_CE);

    IoconPinSetMode(IOCON, IOCON_LED_ALIVE, PIN_MODE_INACTIVE);
    GpioSetPinState(LPC_GPIO_PORT, 0, PIN_LED_ALIVE, false);
    GpioSetPinDIROutput(LPC_GPIO_PORT, 0, PIN_LED_ALIVE);

    IoconPinSetMode(IOCON, IOCON_LED_ACT, PIN_MODE_INACTIVE);
